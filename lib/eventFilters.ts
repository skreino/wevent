...
export function isWeekendEvent(event: Event) {
  const date = new Date(`${event.date}T${event.timeStart || '00:00'}`)
  const day = date.getDay()
  const hour = Number((event.timeStart || '00:00').split(':')[0])
  return day === 0 || day === 6 || (day === 5 && hour >= 18)
}

export function filterEvents(allEvents: Event[], filters: EventFilters) {
  const now = new Date()
  ...
  return allEvents
    .filter((event) => {
      ...
      if (filters.date === 'weekend' && !isWeekendEvent(event)) return false
      return true
    })
    .sort((a, b) => Number(b.featured) - Number(a.featured) || a.date.localeCompare(b.date) || a.timeStart.localeCompare(b.timeStart))
}
